...
class TestPomodoroApp(unittest.TestCase):
    ...
    def test_timer_ends_work_session(self, mock_after, mock_showinfo):
        self.app.remaining_time = 0
        self.app.update_timer()
        mock_showinfo.assert_called_with("Pomodoro Timer", "Work session is over! Time for a break.")
        self.assertFalse(self.app.is_work_time)
        self.assertEqual(self.app.remaining_time, self.app.break_duration)

    @patch('tkinter.messagebox.showinfo')
    @patch('tkinter.Tk.after')
    def test_timer_ends_break_session(self, mock_after, mock_showinfo):
        self.app.is_work_time = False
        self.app.remaining_time = 0
        self.app.update_timer()
        mock_showinfo.assert_called_with("Pomodoro Timer", "Break's over! Time to work.")
        self.assertTrue(self.app.is_work_time)
